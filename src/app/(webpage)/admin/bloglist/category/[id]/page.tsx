...
const ModifyCategory = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategoryData = async () => {
      try {
        const response = await fetch(`/api/blog/Category/getCategoryById/${params.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/blog/Category/updateCategory/${params.id}`, {
        method: 'PUT',
        body: formData,
    ...
    router.push('/admin/bloglist/category')
    } catch (error) {
